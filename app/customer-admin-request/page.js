...
import { useGetcustomergetplanerequestadmin } from "@/helpers/hooks/mamAdmin/customergetplanerequestadmin";
import {
  Autocomplete,
  Box,
  Chip,
...
const Page = () => {
  ...
  const { data, refetch, isLoading } = useGetcustomergetplanerequestadmin(
    page + 1,
    rowsPerPage,
    activeStatus,
  );
  ...
  const handleRowClick = (params) => {
    router.push(`${Routes.customeradminplandetails}${params.row.id}`);
  };
