...
import { useExportCustomers } from "@/helpers/hooks/report-export-apis/reportExportAPI";
import { useGetreportgetCustomer } from "@/helpers/hooks/trainersectionapi/reportgetcustomer";
import { Box, Typography, Paper, Button } from "@mui/material";
import dayjs from "dayjs";
import { Download } from "lucide-react";
import React, { useEffect, useState } from "react";

const CustomerReferralDetails = ({ start_date, end_date, dateRange }) => {
  ...
  const { mutate: exportCustomer, isLoading: iscustomerExporting } =
    useExportCustomers();
  const {
    data: customersData,
    isFetching: isFetchingcustomersData,
    refetch: refetchcustomersData,
  } = useGetreportgetCustomer(
    pagination?.page + 1,
  ...
  const rows = customersData?.data?.page_data?.map((item, index) => ({
    id: item.id,
      ...
      >
        <Box flex={1} overflow={"auto"}>
          <CustomTable
            columns={columns}
            rows={rows}
            loading={isFetchingcustomersData}
            hideFooter={false}
            pagination={{
              page,
              rowsPerPage,
              totalRows: customersData?.data?.page_information?.total_data || 0,
            }}
...
export default CustomerReferralDetails;
