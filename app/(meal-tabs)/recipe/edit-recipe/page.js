...
export default function RecipePreview() {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useGetFoodAllRecipe(page, rowsPerPage, search, type);

 
  const tempRecipeData = templateRecipesAllData?.data?.page_data || [];
            ...
            <div className="flex flex-col items-center">
              <button
                ...
                className={`p-3 rounded-lg text-white text-sm font-medium transition-colors duration-200 ${
                  !isRecipeCreating && selectedTemplate
                    ? "bg-green-600 hover:bg-green-700"
                    : "bg-gray-400 cursor-not-allowed"
                }`}
                disabled={isRecipeCreating || !selectedTemplate}
                onClick={() => setIsTemplateRecipe(true)}
              >
                {isRecipeCreating ? (
