...
import { useGetTrainersPayments } from "@/helpers/hooks/report-export-apis/gettrainerpaymentslist";
...
const TrainerPaymentListTable = ({
  open,
  trainerPaymentData,
  start_date,
}) => {
   const { mutate: exportTrainerPayment, isLoading: iscustomerpaymentExporting } =
      useExportpaymentRole();
       const handleExportTrainerPayment = () => {
      exportTrainerPayment(
        { start_date:start_date,role:"trainer" },
        {
          onSuccess: (res) => {
  ...
  const isReadyToFetch =
    open && !!trainerPaymentData?.customer_ids?.length && !!start_date;
  ...
  const fetchCustomerPayments = () => {
    const payload = {
      ...
      customer_ids: trainerPaymentData?.customer_ids,
    };
  ...
  return (
    <Box height="100%" width="100%" display="flex" flexDirection="column">
      ...
      >
        ...
        <Button variant="contained" color="success" startIcon={<Download />} onClick={handleExportTrainerPayment}>
          Export CSV
        </Button>
      </Box>
        ...
        <Box flex={1} overflow="auto">
          <CustomTable
            ...
            hideFooter={false}
              ...
              totalRows:
                Totalrowdata?.data?.page_information?.total_data || 0,
            }}
...
export default TrainerPaymentListTable;
