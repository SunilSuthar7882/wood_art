...
export default function EditDietPlanPage() {
  ...
  const plan_type = mealPlanData?.data?.type;
  ...
  const handleConfirmSwap = () => {
    handleSwapFood(); // directly use it
  };
  ...
  const handleAddFavMealInPlan = (favoriteId) => {
  ...
  // useEffect(() => {
  //   const storedMode = localStorage.getItem("isEditMode");
  //   if (storedMode !== null) {
  //     setIsEditPlanMode(storedMode === "true");
  //     // clear after using if you don’t want it persistent
  ...
  const validateCurrentDay = (day) => {
    const { valid, reason } = isCurrentDayValid(day);
    ...
    if (!valid) {
      switch (reason) {
        ...
        case "incomplete_servings":
          ...
          break;
        default:
          showSnackbar("⚠️ Add at least one slot.", "error");
      }
    }
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <h1 className="text-2xl font-bold mb-0">
          <button onClick={() => router.back()} className="flex items-center">
            <Image
              ...
              alt="back-icon"
            />
            {role === "customer" && plan_type === "general"
              ? "Meal Plan"
              : "Edit Meal Plan"}
          </button>
        </h1>
      ...
      <div className="m-4 h-full bg-white rounded-2xl shadow-md p-4 flex flex-col flex-1 overflow-auto">
        {/* Plan Header */}
          ...
          <div className="flex justify-between items-start">
            <div>
              <div className="text-xl font-sans font-semibold">
                ...
                {localPlanData?.is_draft && (
                  <span className="px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-yellow-600 capitalize">
                    Draft
                  </span>
                )}
              </div>
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              {((role === "customer" && type === "self_service") ||
                ...
                isEditPlanMode && (
                  <button
                    onClick={() => setIsEditPlanMode(false)}
        ...
        <div className=" flex justify-between align-middle items-center">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!validateCurrentDay(currentDay)) return; 
      ...
      <CopyDaysModal
        ...
        activeDay={activeStep}
        planDays={localPlanData?.days}
        totalDays={localPlanData?.number_of_days || 1}
        // onConfirm={(selectedDays, replaceExisting) => {
        //   console.log("Copy day", activeStep, "to", selectedDays, {
        //     replaceExisting,
        //   });
        // }}
      />
