...
const SwapFood = ({
  ...
  console.log("foodIdToReplace", foodIdToReplace);
  const { showSnackbar } = useSnackbar();
  //   const { data: foodData } = useGetFoodServings({ food_id: foodId });
  //   console.log("foodDatafoodDatafoodData",foodData)
  const { reset } = useForm({
    defaultValues: {
      plan_name: "",
      number_of_days: "",
  ...
  useEffect(() => {
    if (!isSwapOpen) {
      setSearchValue(""); // clear search when modal closes
    }
  }, [isSwapOpen]);
  ...
  return (
    <>
      <Dialog open={!!isSwapOpen} onClose={handleClose} fullWidth maxWidth="sm">
        <DialogTitle
          sx={{
            display: "flex",
            alignItems: "center",
