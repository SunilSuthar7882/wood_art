...
export default function AddEditDietPlanModal({
  isOpen,
  setIsOpen,
  customerAdminId,
  customerTrainerId,
  ...
  const fetchAllTemplatePlans = async ({ queryKey }) => {
    ...
    const { searchText, numberOfDays, categoryId, minCalories, maxCalories } =
      filters || {};
  ...
  const { data: templatePlansAllData, refetch: refetchTemplates } = useQuery({
    queryKey: [
      "all-template-plans",
      {
        searchText: search,
  ...
  //     //   setValue("plan_name", "");
  //     setValue("number_of_days", tempPlansAllData.number_of_days);
  //     //   setValue("category_ids", []);
  //     setLittleTemplateData(tempPlansAllData);
  //   }
  // }, [tempPlansAllData]);
  ...
  return (
    <Dialog
      ...
      fullWidth
      maxWidth="md"
      // sx={{maxWidth:"400px", alignItems:"center", justifyContent:"center"}}
    >
      <DialogTitle
        sx={{
          display: "flex",
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        >
          ...
          />
          <label className="input-label block ml-1 mb-1">Number Of Day*</label>
          ...
          <div className="flex gap-4">
            <div className="w-1/2">{/* Number Of Days */}</div>
            {/* <div className="w-1/2">
          ...
          <label className="input-label block ml-1 my-1">
            My Personal Meal Plan Profile
          </label>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div className="md:col-span-1 flex justify-end">
              <Button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-md bg-blue-600 text-white font-medium hover:bg-blue-700"
              >
                <SquarePen size={16} />
                {isEdit ? "Save" : "Edit"}
              </Button>
          ...
          <label className="input-label block mb-3 mt-3">
            Select Meal Plan
          </label>
          ...
          {/* <label className="inline-flex items-center space-x-2">
            <input
              type="checkbox"
              checked={showTemplateOptions}
              onChange={(e) => setShowTemplateOptions(e.target.checked)}
              className="h-4 w-4 accent-green-600 rounded border-gray-300"
            />
            <span className="text-sm font-medium ">Select Template Plan</span>
          </label>
            ...
            {(tempPlansAllData || [])
              ...
              .map((plan, index) => (
                <li
                  ...
                  className={`px-3 py-2 text-sm cursor-pointer transition-colors duration-150 border-l-2 ${
                    selectedTemplate?.id === plan.id
                      ? "bg-green-50 border-green-500"
                      : "bg-white border-transparent"
                  } hover:bg-green-100`}
                >
                  <div className="font-medium text-gray-800 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span>{plan.name}</span>
                      <span className="text-gray-400">|</span>
                      <span>{plan.number_of_days} Days</span>
                    </div>
                    {plan.user?.full_name && plan.user?.role && (
                      <div className="text-[12px] text-gray-500 italic">
                        Created by: {plan.user.full_name}{" "}
                        {`(${plan.user.role})`}
                      </div>
                    )}
                  </div>
